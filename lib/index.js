...
function registerRefreshTool(ctx, holder, refreshed, pricingFile, config, rebuild) {
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const updated = applyRefreshed(refreshed, parseRefreshedPricing(await extractWithLlm(ctx, {
				provider,
				model,
				prompt: buildExtractPrompt(models, renderSearchContent(await ctx.web.search({
...
async function persistRegistry(pricingFile, models, version) {
