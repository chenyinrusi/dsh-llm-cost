...
*/
async function resolveExtractionCandidates(ctx, holder, config) {
	...
	if (config.refreshProvider !== void 0 && config.refreshModel !== void 0) push(config.refreshProvider, config.refreshModel);
	const discovered = [];
	...
	for (const candidate of sortExtractionCandidates(holder.registry, discovered)) push(candidate.provider, candidate.model);
	return candidates;
}
/**
