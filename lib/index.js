...
*/
function costUsd(usage, entry) {
	const perM = (tokens, price) => tokens > 0 && price !== void 0 && price > 0 ? tokens / 1e6 * price : 0;
	return perM(usage.inputTokens, entry.inputPerM) + perM(usage.cacheReadTokens ?? 0, entry.cacheReadPerM) + perM(usage.cacheWriteTokens ?? 0, entry.cacheWritePerM) + perM(usage.outputTokens, entry.outputPerM);
}
/**
...
*/
function resolveAndCost(registry, model, provider, usage) {
	...
	return {
		costUsd: costUsd(usage, resolution.entry),
		resolution
	};
}
...
function recomputeTotals(steps) {
	let totalCostUsd = 0;
	let pricedSteps = 0;
	let unpricedSteps = 0;
	let inputTokens = 0;
	...
	for (const step of steps) {
		...
		if (step.costUsd === null) unpricedSteps += 1;
		else {
			pricedSteps += 1;
		...
		if (existing === void 0) byModelMap.set(key, agg);
	}
...
function applyCostEvent(state, event, registry) {
	...
	const { costUsd } = resolveAndCost(registry, model ?? "", provider ?? "", usage);
	const record = {
		turn,
		step,
		model,
	...
	const steps = last !== null && last.turn === turn && last.step === step ? state.steps.slice(0, -1).concat([record]) : state.steps.concat([record]);
	return {
		route,
...
function buildSearchQuery(models) {
	return `current API price per million tokens 2026: "${models.slice(0, 12).join("\" OR \"")}" input output cache`;
}
/**
...
function applyRefreshed(registry, refreshed) {
	const updated = [];
...
function apply(ctx, config = {}) {
	const holder = { registry: mergeRegistries(DEFAULT_PRICING, config.pricing) };
...
function registerRefreshTool(ctx, holder, pricingFile, config) {
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			const models = Array.isArray(args.models) && args.models.length > 0 ? args.models : Object.keys(holder.registry.models);
			if (models.length === 0) return {
				updatedModels: [],
				count: 0
			};
			const prompt = buildExtractPrompt(models, renderSearchContent(await ctx.web.search({
				query: buildSearchQuery(models),
				maxResults: 10
			}, exec.signal)));
			const provider = config.refreshProvider;
			const model = config.refreshModel;
			if (provider === void 0 || model === void 0) throw new Error("llm-cost refresh needs refreshProvider + refreshModel in the plugin config (the model that extracts prices from search results)");
			const refreshed = parseRefreshedPricing(await extractWithLlm(ctx, {
				provider,
				model,
				prompt,
				signal: exec.signal
...
async function persistRegistry(pricingFile, registry) {
